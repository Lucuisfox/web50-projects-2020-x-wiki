...
def create_listing(request):
    ...
    if request.method == "POST":
        ...
        if request.POST.get('image_link'):
            ...
            item.image_link = "https://media.istockphoto.com/id/163208487/photo/male-coat-isolated-on-the-white.jpg?s=612x612&w=0&k=20&c=3Sdq5xnVS2jOYPNXI6JLwAumzyelcP_VgKVW0MVUhwo="
...
def watchlists(request):
    watchlists = Watchlist.objects.filter(user=request.user.username).select_related()
    print(watchlists)
    empty = False
    if len(watchlists) == 0:
        empty = True
